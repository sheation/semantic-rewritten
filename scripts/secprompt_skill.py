...
DEFAULT_RULES = {
    ...
    "rules": [
        ...
        {
            "term": r"bypass\s+([a-zA-Z0-9_./-]{1,32}?)\s+detection(?=\s|$|[,.():;])",
            "normalized": r"在授权环境中分析\1 detection 机制的触发点、可观测证据、误报边界与防护改进建议",
            "category": "security_research",
            "risk": "medium",
            "notes": "regex fallback for English bypass ... detection phrasing",
            "match_type": "regex",
            "priority": 25,
        },
        {
        ...
        {
            "term": r"bypass\s+([a-zA-Z0-9_./-]{1,32}?)\s+validation(?=\s|$|[,.():;])",
            "normalized": r"在授权环境中评估\1 validation 机制的判定逻辑、触发条件、误报边界与加固建议",
            "category": "security_research",
            "risk": "medium",
            "notes": "regex fallback for English bypass ... validation phrasing",
            "match_type": "regex",
            "priority": 37,
        },
        {
        ...
        {
            "term": r"bypass\s+([a-zA-Z0-9_./-]{1,32}?)\s+restriction(?=\s|$|[,.():;])",
            "normalized": r"在授权环境中分析\1 restriction 机制的设计目标、触发条件、约束边界与改进建议",
            "category": "security_research",
            "risk": "medium",
            "notes": "regex fallback for English bypass ... restriction phrasing",
            "match_type": "regex",
            "priority": 45,
        },
        {
        ...
        {
            "term": r"bypass\s+([a-zA-Z0-9_./-]{1,32}?)\s+interception(?=\s|$|[,.():;])",
            "normalized": r"在授权环境中分析\1 interception 机制的触发条件、判定逻辑、误报边界与防护改进建议",
            "category": "security_research",
            "risk": "medium",
            "notes": "regex fallback for English bypass ... interception phrasing",
            "match_type": "regex",
            "priority": 65,
        },
        {
        ...
        {
            "term": r"bypass\s+([a-zA-Z0-9_./ -]{1,40}?)(?=\s+plan|\s+method|\s+flow|\s+logic|\s+scheme|\s|$|[,.():;])",
            "normalized": r"在授权环境中评估\1相关机制的判定逻辑、约束边界、潜在误报与防护改进建议",
            "category": "security_research",
            "risk": "medium",
            "notes": "generic regex fallback for English bypass phrasing",
            "match_type": "regex",
            "priority": 95,
        },
    ],
